...
class ExecutionEngineSDK {
    ...
    async checkTargetSafety(target) {
        ...
        try {
            const res = await fetch(`https://api.gopluslabs.io/api/v1/address_security/${target}?chain_id=1`);
            ...
            // Fallback to local checks silently if GoPlus endpoint is rate-limited or offline
        }
